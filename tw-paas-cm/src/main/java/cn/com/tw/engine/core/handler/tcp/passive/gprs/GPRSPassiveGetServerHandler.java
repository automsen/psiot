package cn.com.tw.engine.core.handler.tcp.passive.gprs;
...
@Sharable
public class GPRSPassiveGetServerHandler extends ChannelInboundHandlerAdapter {
	
	private static Logger logger = LoggerFactory.getLogger(GPRSPassiveGetServerHandler.class);
	...
	public GPRSPassiveGetServerHandler(){
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) { // (2)
		...
		try {
			...
			if ("09000090".equals(dataField)){
				...
				//暂时加在这里， 是因为设备网关返回数据错位，导致匹配不上，采集不了数据，直接将采集的数据放入mq
				...
				if(StringUtils.isEmpty(gwId)) {
					//通过仪表ID 获取网关ID
					gwId = channelManaFactory.getGwIdByChannelId(ctx.channel().id().asLongText());
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) {
		...
		if (!StringUtils.isEmpty(clientId)) {
			asyncTask(clientId, TermStatus.close.getValue());
			logger.error("e = {} ", new Object[]{SysCons.LOG_ERROR, clientId, cause.toString()} );
		}
