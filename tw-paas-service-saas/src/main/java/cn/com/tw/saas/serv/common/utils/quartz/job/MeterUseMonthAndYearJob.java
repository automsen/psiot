...
public class MeterUseMonthAndYearJob implements Job{
	
    private static Logger logger = LoggerFactory.getLogger(MeterUseMonthAndYearJob.class);
	@Autowired
	private TaskService taskService;
	
	private MeterUseQuantumService meterUseQuantumService;
	
	public MeterUseMonthAndYearJob(){
	...
	public void execute(JobExecutionContext ctx)
		...
		logger.debug(">>>>>>>>>>>>>>>>>>>>>>>start meterUseMonthAndYearJob  [{}]", sdf.format(lastTime));
       //获取任务的group名字
		...
		try {
			...
			logger.error("job-meterUseMonthAndYearJob job TaskId:({}) Task is undefiend exception: {}",task.getTaskName(), e.toString());
			throw new JobExecutionException();
		}
		try {
			meterUseQuantumService.meterUseMonthAndYearJob(sdf.format(lastTime));
			//任务执行成功更新任务状态
			...
			logger.info("<<<<<<<<<<<<< end meterUseMonthAndYearJob");
		} catch (Exception e){
			logger.error("job-meterUseMonthAndYearJob job TaskId:({}) job handle failed: {}",task.getId(), e.toString());
			//更新任务状态为执行失败
