...
public class CmdApiController {
	...
	private static Logger logger = LoggerFactory.getLogger(CmdApiController.class);
	...
	public CmdResponse acceptCmd(@RequestBody CmdRequest req){
		...
		try {
			...
			logger.debug(SysCons.LOG_END + " acceptCmd req success");
			return resp;
		...
		return CmdResponse.retn(ResultCode.UNKNOW_ERROR);
