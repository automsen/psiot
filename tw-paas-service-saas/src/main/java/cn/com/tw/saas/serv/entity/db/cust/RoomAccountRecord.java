 ...
 * 房间账户记录
 * 数据库实体
 * 对应t_saas_cust_room_account_record
 */
public class RoomAccountRecord {
...
3，撤销
     ...
     * 购买用电量
     */
    private BigDecimal meterMuch;
    
    
    
    private String meterMuchStr;
    /**
     *操作用户的真实姓名
     */
	 ...
	 * @return
	 */
	public static RoomAccountRecord generateRecord(Room room,Customer cust,OrgUser user,BigDecimal money){
		RoomAccountRecord record = new RoomAccountRecord();
...
//		record.setPayModeCode("");
		...
		record.setBalance(room.getBalance());
		record.setStatus((byte)1);
		record.setRegionId(room.getRegionId());
		record.setRegionNo(room.getRegionNo());
//		record.setRemark("");
//		record.setDetails("");
		if (null!=cust){
	...
	public BigDecimal getMeterMuch() {
		return meterMuch;
	}
	public void setMeterMuch(BigDecimal meterMuch) {
		this.meterMuch = meterMuch;
	}
	...
	public String getMeterMuchStr() {
		return meterMuchStr;
	}
	public void setMeterMuchStr(String meterMuchStr) {
		this.meterMuchStr = meterMuchStr;
	}
