layui.config({
	...
	form.on('submit(tijiao)', function(data){
		...
		if(p.orgId != orgId  && orgId != '' && orgId != null){
			p.parentOrgId = orgId
		}
		var orgEquipment = JSON.stringify(p);
        T.reqAjax({btnId:"subBtn", method:"put", param:orgEquipment, url:"org",contentType: "application/json",dataType: 'json',success:function(res){
        	layer.msg('修改成功');
        	data.form.reset();
        },error:function(res){
	...
	$("#s_form").loadToDom({url:"org/"+orgId});
});