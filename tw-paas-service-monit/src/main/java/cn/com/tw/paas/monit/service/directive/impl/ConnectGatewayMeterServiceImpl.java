...
import cn.com.tw.paas.monit.service.directive.ConnectGatewayMeterService;
 ...
 */
@Service
public class ConnectGatewayMeterServiceImpl implements ConnectGatewayMeterService {

	private static Logger logger = LoggerFactory.getLogger(ConnectGatewayMeterServiceImpl.class);
	 ...
	 */
	@Override
	public PageCmdResult connectGatewayMeter(String netEquipAddr,CmdCallBack callback ,String commAddrs) {
		...
		/**
		 * 根据采集器地址 查询设备表内与采集器绑定的所有仪表
		 */
		String[] equipmentExpands = commAddrs.split(",");
		/*OrgEquipment orgEquipment2 = new OrgEquipment();
		orgEquipment2.setNetEquipAddr(netEquipAddr);
		List<OrgEquipmentExpand> equipmentExpands = orgEquipmentMapper.selectOrgEquipmentAll(orgEquipment2);*/
		
		if (terminalEquip == null) {
			logger.error("connectGatewayMeter handle error netEquipAddr is undefiend!");
			return null;
		}
		...
		//采集器绑仪表
		inn1 = CmdInnEXEUtil.connectGatewayInnExe(commandExe, PriceEnum.connectGatewayMeter, terminalEquip, 1,new Byte("10"),equipmentExpands);
		inns.add(inn1);
